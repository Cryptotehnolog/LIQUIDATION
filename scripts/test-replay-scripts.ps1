...
Assert-True ($collectWindow.Contains("300 + `$PostWindowGraceSeconds")) "collect-paper-replay-window.ps1 must require enough runtime for a complete 5-minute Polymarket window"
Assert-True ($collectWindow.Contains("MaxRuntimeSeconds must be at least")) "collect-paper-replay-window.ps1 must fail fast on incomplete Polymarket window runtime"
...
Assert-True ($untilSignalBuilt.Contains("[int]`$MaxRuntimeSeconds = 330")) "run-until-signal-built.ps1 must default to a complete 5-minute Polymarket window plus grace"
...
Assert-True ($untilSignalBuiltAggregate.Contains("[int]`$MaxRuntimeSeconds = 330")) "run-until-signal-built-aggregate.ps1 must default to a complete 5-minute Polymarket window plus grace"
...
Assert-True ($untilSignalBuiltAggregate.Contains('"-ReplayArtifactListPath"')) "run-until-signal-built-aggregate.ps1 must pass combined replay artifacts through a manifest"
Assert-True ($untilSignalBuiltAggregate.Contains('".artifacts.txt"')) "run-until-signal-built-aggregate.ps1 must write a replay artifact manifest next to the combined analysis"
...
Assert-True ($entryFillAnalyzer.Contains("[string]`$ReplayArtifactListPath")) "analyze-entry-fill-diagnostics.ps1 must accept replay artifact manifest paths"
Assert-True ($entryFillAnalyzer.Contains("Replay artifact list not found")) "analyze-entry-fill-diagnostics.ps1 must fail clearly when a replay artifact manifest is missing"
