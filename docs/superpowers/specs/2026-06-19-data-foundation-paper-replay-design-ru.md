...
Raw payloads остаются в TimescaleDB для MVP. Если measured storage growth
пересечет configured limit, follow-up design переносит cold raw payload blobs в
S3-compatible object store, а в TimescaleDB оставляет только content hashes и
object references.
...
- `trade_cross`: default conservative model. Polymarket buy limit fills только
  ...
  limit price.
- `book_depth_cross`: разрешается только при наличии recorded order book depth.
  Buy fills, если recorded ask depth at or below limit price не меньше simulated
  order size; sell fills, если recorded bid depth at or above limit price не
  меньше simulated order size.
...
`cargo deny` является follow-up после появления dependency set; делать license и
advisory policy обязательными до scaffold workspace добавит noise без улучшения
первого design decision.
...
- mock load tests показывают, что recorder выдерживает configured burst rates
  без data loss;
