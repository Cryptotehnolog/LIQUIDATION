...
Для controlled paper replay с dashboard используйте единый operator script:
...
Скрипт делает ровно operator sequence:

- ждёт replay-ready window через `wait-for-liquidation-replay.ps1`;
- собирает Bybit/Binance/OKX liquidation data, Polymarket UP/DOWN market data
  и Hyperliquid hedge market data;
- запускает `replay preflight`, затем `replay run`;
- обновляет `.cache/replay/latest-polymarket-baseline.json` и
  `.cache/replay/latest-polymarket-market.json`;
- запускает read-only dashboard через `scripts/start-dashboard.ps1` и
  открывает `http://127.0.0.1:18080/`.
